...
export const posts = [
  {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Sounds clean. In practice, after years of running this at scale, two classes of predictions had quietly piled up in our databases: predictions that were never properly processed, ingested but incomplete because the model’s predictions got better over time and initial processed prescriptions were not processed properly and predictions where the model was confident enough that routing them to a human reviewer was just ceremony. The model was right. No one needed to check.' },
      ...
      { type: 'paragraph', text: 'You also can’t just fire-and-forget at full throttle. Our downstream AI service had a hard concurrency limit, throw more than ten simultaneous requests at it and it starts queing them, creating a never ending congestion And each prescription needed order details fetched from another service’s database before it could be processed. This database is highly sensitive and even a small increase in its load would be actual production performance difference for normal end user.' },
      ...
      { type: 'paragraph', text: 'The digitization service scans its own job table to find candidates for reingestion. But the prediction ID, the internal identifier the prediction service uses to track each record is completely foreign to the service. It doesn’t own that concept.' },
...
prediction_id = existing ? existing.id : create_prediction(candidate)`,
      ...
      { type: 'paragraph', text: 'The cron runs for 55 minutes and processes roughly 2000 prescriptions per run. If the pod gets killed halfway through, which happens, you don’t want to restart from the beginning. That’s double-processing, wasted VLM calls, and a backlog that never actually shrinks.' },
...
after batch:
  save_cursor_to_db(current_position)`,
      ...
      { type: 'paragraph', text: 'Order details for each prescription live in a separate order service, different team, different database, different tech stack. The original design called a REST API for each prescription. That’s fine at tens of records. At ten concurrent prescriptions per batch running for 55 minutes, it becomes a lot of HTTP overhead.' },
      { type: 'paragraph', text: 'We switched to a direct, read-only database connection to the order service database replica. The query joins two tables, pulls back the relevant SKUs from a JSON column, and returns in under 10 milliseconds. Same result, a fraction of the overhead.' },
...
for prescription in batch:
  ...
  async:
    try:
      ...
      { type: 'paragraph', text: 'The first run hit the database on a cold start, no cached cursor, no warm-up, just 55 minutes of work.' },
      {
        type: 'stats',
        items: [
          { label: 'Runtime', value: '~55 min' },
          { label: 'Prescriptions processed', value: '2040' },
          { label: 'Successful', value: '2039' },
          { label: 'Failed', value: '1' },
        ],
      },
      { type: 'paragraph', text: 'At this rate, roughly 48936 prescriptions per day get reprocessed. The multi-million record backlog clears in weeks. After that, the cron stays active but mostly idle each hour, it has nothing left to do but keep up with fresh volume.' },
